...
def _issuer() -> str:
    ...
    raise ValueError("Keycloak is not configured. Set KEYCLOAK_ISSUER or KEYCLOAK_URL and KEYCLOAK_REALM.")
...
async def authenticate(headers: dict) -> dict:
    ...
    try:
        ...
        raise Auth.exceptions.HTTPException(status_code=401, detail="Invalid token") from exc

    identity = claims.get("sub")
    if not identity:
        raise Auth.exceptions.HTTPException(status_code=401, detail="Token missing subject")

    display_name = claims.get("preferred_username") or claims.get("name") or claims.get("email") or identity
