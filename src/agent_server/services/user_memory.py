...
async def get_user_memory_context(session: AsyncSession, user_id: str) -> str:
    snapshot = await session.scalar(
        select(UserMemorySnapshot).where(UserMemorySnapshot.user_id == user_id)
    )
...
async def _rebuild_snapshot(
    session: AsyncSession, user_id: str, output_data: Any
) -> None:
    ...
    facts = [
        memory for memory in reversed(result.all()) if _memory_is_snapshot_ready(memory)
    ][-MAX_FACTS:]
    ...
    metadata.update(
        {
            ...
            "confidence": "medium"
            if evidence_count >= IMPLICIT_PROMOTION_THRESHOLD
            else "low",
...
def _message_text(
    message: Any, *, human_only: bool = False, assistant_only: bool = False
) -> str:
